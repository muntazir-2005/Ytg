...
NSString* InitOffsetForPatchHook(const char* machoPath, uint64_t vaddr, const char* patch)
{
    ...
    if(newPath) {
        ...
        if(!macho) return [NSString stringWithFormat:NSSENCRYPT("Can't find file:\n Documents/PATCH-NONJB/%s"), machoPath];
    } else {
        macho = load_macho_data(path);
        if(!macho) return [NSString stringWithFormat:NSSENCRYPT("Can't read file:\n.app/%s"), machoPath];
    }
    ...
    while(true) {
        ...
        if(!macho) {
            return NSSENCRYPT("add_hook_section error!");
        }
    }
    
    if(cryptid != 0) {
        return NSSENCRYPT("this app is not decrypted!");
    }
    
    if(!text_seg || !data_seg) {
        return NSSENCRYPT("can not parse machO file!");
    }
    ...
    if(!funcData) {
        return NSSENCRYPT("Invalid offset!");
    }    
    ...
    if(patch && patch[0]) {
        ...
        if(!hex2bytes(patch, (uint8_t*)patch_bytes+vaddr%4))
            return NSSENCRYPT("The bytes to patch are incorrect!");

    } else if(vaddr % 4) {
        return NSSENCRYPT("The offset is not aligned!");
    }
    ...
    for(int i=0; i<STATIC_HOOK_CODEPAGE_SIZE/sizeof(StaticInlineHookBlock); i++)
    {
        if(hookBlock[i].hook_vaddr==funcRVA)
        {
            if(patch && patch[0] && hookBlock[i].patch_hash!=calc_patch_hash(vaddr, patch))
                return NSSENCRYPT("The bytes to patch have changed, revert to original file and try again");
            
            if(newPath)
                return NSSENCRYPT("The offset is already patched");
        ...
        if( funcRVA>hookBlock[i].hook_vaddr &&
           ( funcRVA < (hookBlock[i].hook_vaddr+hookBlock[i].hook_size) || funcRVA < (hookBlock[i].hook_vaddr+hookBlock[i].patch_size) )
            ...
            return NSSENCRYPT("The offset is occupied!");
        }
    ...
    if(!hookBlockRVA) {
        return NSSENCRYPT("HOOK count full!");
    }
    ...
    {
        return NSSENCRYPT("can not patch the offset");
    }
    ...
    NSString* savePath = [NSString stringWithFormat:NSSENCRYPT("%@/Documents/PATCH-NONJB/%s"), NSHomeDirectory(), machoPath];
...
BOOL PatchOffset(const char* machoPath, uint64_t vaddr, const char* patch, bool isOn)
{
    ...
    if (!base) {
        //NSLog(NSSENCRYPT("cannot find module!"));
        return NO;
    }
    
    StaticInlineHookBlock* hookBlock = find_hook_block(base, vaddr&~3);
    if (!hookBlock) {
        //NSLog(NSSENCRYPT("cannot find hook block!"));
        return NO;
    }
    
    if (hookBlock->patch_hash != calc_patch_hash(vaddr, patch)) {
        //NSLog(NSSENCRYPT("code patch bytes changed!"));
        return NO;
    }
